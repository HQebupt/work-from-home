...
def step1(L, a ,b):
    ...
    result1 = solve([116.0 * ((Y / 100.0) ** (1.0/3.0)) - 16.0 - L,
        500 * ( (X / 98.07) ** (1.0 / 3.0) - (Y / 100.0) ** (1.0 / 3.0) ) - a,
        200 * ( (Y / 100.0) ** (1.0 / 3.0) - (Z / 118.22) ** (1.0 / 3.0) ) - b], [X, Y, Z]);
    for item in result1:
        X,Y,Z = item;
...
def step2(Y):
    global V
    print("###########[Step2]###########")
    V  = solve(1.1914 * V - 0.22533 * V * V + 0.23352 * (V ** 3) - 0.020484 * ( V ** 4) + 0.00081939 * ( V ** 5) - Y, V)
